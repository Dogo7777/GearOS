...
dnf5 install -y --setopt=install_weak_deps=False \
    ...
    ptyxis nautilus fontconfig gnome-software \
    cascadia-fonts-all google-roboto-fonts \
    nautilus-extensions adwaita-icon-theme \
    plasma-nm plasma-pa powerdevil \
    xdg-desktop-portal xdg-desktop-portal-kde \
    kvantum

# Instala tema Kvantum
...
dnf5 install -y --skip-unavailable \
    ...
    fuse fuse-libs fuse3 fuse3-libs squashfs-tools \
    nautilus-python nautilus-open-any-terminal \
    unzip p7zip p7zip-plugins \
    pciutils upower xdg-utils kdeconnect android-tools ark nano less \
    google-noto-emoji-color-fonts langpacks-core-pt_BR \
    langpacks-pt_BR papirus-icon-theme langpacks-fonts-pt \
...
export INITRD=no
dnf5 remove -y kernel kernel-core kernel-modules || true
...
grep -q '^LOGO=' /etc/os-release && \
    sed -i 's|^LOGO=.*|LOGO=/usr/share/pixmaps/gearos.png|' /etc/os-release || \
    echo 'LOGO=/usr/share/pixmaps/gearos.png' >> /etc/os-release
...
# 9. GTK/GNOME SETTINGS
mkdir -p /etc/dconf/db/local.d
printf '[org/gnome/desktop/wm/preferences]\nbutton-layout='"'"'appmenu:minimize,maximize,close'"'"'\n' \
    > /etc/dconf/db/local.d/01-gearos

# Ícone Papirus e nautilus-open-any-terminal com Ptyxis
printf '[org.gnome.desktop.interface]\nicon-theme='"'"'Papirus'"'"'\n' \
    > /usr/share/glib-2.0/schemas/99-gearos-icons.gschema.override
printf '[com.github.stunkymonkey.nautilus-open-any-terminal]\nterminal='"'"'ptyxis'"'"'\n' \
    > /usr/share/glib-2.0/schemas/99-gearos-nautilus.gschema.override
...
# 10. FONTES - MesloLGS NF para Powerlevel10k
...
# Homebrew em /home/linuxbrew para ser gravável em /var
mkdir -p /var/home/linuxbrew
ln -sf /var/home/linuxbrew /home/linuxbrew
git clone --depth=1 https://github.com/Homebrew/brew /var/home/linuxbrew/.linuxbrew
mkdir -p /var/home/linuxbrew/.linuxbrew/bin
ln -sf ../Homebrew/bin/brew /var/home/linuxbrew/.linuxbrew/bin/brew || true
...
# CONFIGURAÇÃO DE PADRÕES E SERVIÇOS
RUN <<CONFIGEOF
set -ex
...
# KDE global - tema escuro + AurowaitaDark + Roboto + Ptyxis
...
gear() {
    ...
    _exportar_app() {
        ...
        if [[ -n "$desktop_file" ]]; then
            ...
            echo -e "${c_green}Atalho integrado com sucesso!${c_reset}"
        fi
    }
    ...
    case "$1" in
        install)
            ...
            if [[ -f "$pkg" && "$pkg" == *.deb ]]; then
                ...
                echo -n -e "${c_yellow}Nome do app para exportar: ${c_reset}"; read app_name
                [[ -n "$app_name" ]] && _exportar_app "gear-debian" "$app_name" "debian"
                return 0
            fi
        ...
        update)
            ...
            if sudo bootc upgrade; then
                echo -e "${c_green}Pronto! Reinicie para aplicar.${c_reset}"
            else
